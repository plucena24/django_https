...
\pause
\item  Enc= Encryption; AESGCM(256) is 256 bit Advanced Encryption Standard in Galois Counter mode for encryption
\pause
...
\begin{minted}{bash}
 cat COMODORSADomainValidationSecureServerCA.crt COMODORSAAddTrustCA.crt > ocsp.crt
\end{minted}
\pause
...
\end{minted}
\pause
\item Note that this was a non-standard extension to html. There is a standardized way (see content security policies), but it's not yet supported by all modern browsers
\end{itemize}
\end{frame}
